...
var jsasync = (function() {
	...
	jsasync.prototype._run_hndl = function(si, tar, tar_idx, scp, varg, bra, args) {
		...
		if(typeof(itm) == 'function') {
			...
			if(tar.length > 0) {
				...
				if(bra > 0) {
					return this._run_hndl_next(si, [], [], scp, false, 0, args);
				}
				if(ti >= tar.length) bra = 1;
				hndl.break = this._run_hndl.bind(this, si, tar, tar_idx, scp, true, 1);
				hndl.continue = this._run_hndl.bind(this, si, tar, tar_idx, scp, true, bra);
				return itm.apply(this.self, [hndl].concat(args));
			} else {
				hndl.return = this._run_hndl_next.bind(this, si, [], [], scp, true, 0);
				hndl.branch = this._run_hndl_next.bind(this, si, [], [], scp, true);
				return itm.apply(this.self, [hndl].concat(args));
			}
		} else {
			tar.push(itm);
			tar_idx.push(0);
			return this._run_hndl_next(si, tar, tar_idx, scp, false, 0, args);
		}
	};
	jsasync.prototype._run_hndl_next = function(si, tar, tar_idx, scp, varg, bra, args) {
	...
	jsasync.prototype.run = function(seq, args) {
		var si = [seq, null];
		return this._run_hndl_next(si, [], [], {}, false, 0, args);
	};
...
function f3(hndl) { console.log('f3', arguments); hndl.return(666) }
